...
from src.utils.utils import  mix_rbf_mmd2
...
class MNISTGANModel(LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        if self.objective_type not in ["LSGAN", "Rg_LSGAN", "SGAN", "Rg_SGAN"]:
            raise ValueError(f"Objective type {self.objective_type} is not supported. Please choose from ['LSGAN', 'Rg_LSGAN', 'SGAN',  'Rg_SGAN'].")
        if self.objective_type in ["LSGAN", "Rg_LSGAN"]:
            self.adversarial_loss = torch.nn.MSELoss()
        elif self.objective_type in ["SGAN", "Rg_SGAN"]:
    ...
    def step(self, batch, batch_idx, optimizer_idx=None) -> Tuple[Dict[str, Tensor], Optional[Tensor]]:
        ...
        gen_data_target = MNISTGANModel.gen_data_target(batch_size) if self.objective_type in ["Rg_LSGAN", "LSGAN"] \
            else MNISTGANModel.gen_data_target(batch_size, 0)
        ...
        if optimizer_idx == 0 or not self.training:
            ...
            if self.objective_type in ["Rg_LSGAN", "Rg_SGAN"]:
                loss_real = (self.adversarial_loss(prediction_d, real_data_target) *
                             (0.5 if self.objective_type in ["Rg_LSGAN"] else 1))
                loss_fake = (self.adversarial_loss(prediction_d, gen_data_target) *
                             (0.5 if self.objective_type in ["Rg_LSGAN"] else 1))
                loss =  (loss_real - loss_fake)
            else:
                loss = (self.adversarial_loss(prediction_d, real_data_target) *
                        (0.5 if self.objective_type in ["LSGAN"] else 1))
        ...
        if optimizer_idx == 1 or not self.training:
            ...
            loss = (real_d_error + gen_d_error) * (0.5 if self.objective_type in ["LSGAN", "Rg_LSGAN"] else 1)
