...
class CIFAR10GANModel(LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        if self.objective_type not in ["LSGAN", "Rg_LSGAN", "SGAN", "Rg_SGAN"]:
            raise ValueError(
                f"Objective type {self.objective_type} is not supported. Please choose from ['LSGAN', 'Rg_LSGAN', 'SGAN',  'Rg_SGAN'].")
        if self.objective_type in ["LSGAN", "Rg_LSGAN"]:
            self.adversarial_loss = torch.nn.MSELoss()
        elif self.objective_type == ["SGAN", "Rg_SGAN"]:
    ...
    def step(self, batch, batch_idx, optimizer_idx=None) -> Tuple[Dict[str, Tensor], Optional[Tensor]]:
        ...
        gen_data_target = CIFAR10GANModel.gen_data_target(batch_size) if self.objective_type in ["Rg_LSGAN", "LSGAN"] \
            else CIFAR10GANModel.gen_data_target(batch_size, 0)
